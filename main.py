...
st.warning(“  Streamlit Secrets에 GEMINI_API_KEY를 설정해주세요.”)
...
1.   매수 전략 (1-2줄)
1.   리스크 요인 (1줄)
1.   종합 의견: [강력매수 / 매수 / 관망 / 매도] 중 하나로 답변
“””
...
# RSI 아이콘
rsi = s['extra']['rsi']
rsi_icon = " " if rsi < 30 else (" " if rsi < 50 else (" " if rsi < 70 else " "))

# 업사이드 아이콘
up_icon = " " if upside > 15 else (" " if upside > 7 else " ")
...
# 지표 그리드
rsi_icon = " " if last['RSI'] < 30 else (" " if last['RSI'] < 50 else (" " if last['RSI'] < 70 else " "))
...
if errors:
with st.expander(f”  수집 실패 항목 ({len(errors)}건)”, expanded=False):
for e in errors[-20:]:
st.text(e)
...
“  본 앱은 투자 참고용입니다. 투자 판단과 책임은 본인에게 있습니다.”
“</p>”,
unsafe_allow_html=True
)
